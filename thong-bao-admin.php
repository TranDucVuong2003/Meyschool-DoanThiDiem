...
dashboard_require_access('notifications');
...
<head>
    ...
    <title>Quản lý Thông báo | Meyschool Đoàn Thị Điểm</title>
...
<body>
    ...
    <div class="main">
        <header class="topbar">
            <div class="topbar-title">Quản lý Thông báo</div>
        ...
        <div class="content">
            <div class="page-header">
                <h2>Thông báo</h2>
                <a href="them-thong-bao.php" class="btn btn-primary">+ Thêm thông báo</a>
            </div>
    ...
    <script>
        ...
        function loadNotifs(page) {
            ...
            fetch('api/notifications.php?' + params)
                .then(r => r.json())
                .then(res => {
        ...
        function renderTable(rows, total, page, totalPages) {
            ...
            if (rows.length === 0) {
                tbody.innerHTML = '<tr><td colspan="7" class="empty-state">Chưa có thông báo nào.</td></tr>';
            ...
            tbody.innerHTML = rows.map((r, i) => `
                <tr>
                    ...
                    <td class="thumb-cell">
                        ${r.thumbnail
                            ? `<img src="${escHtml(r.thumbnail)}" alt="">`
                            : '<div class="thumb-placeholder">&#128247;</div>'}
                    </td>
                    <td class="title-cell">
                        <a href="them-thong-bao.php?id=${r.id}">${escHtml(r.title)}</a>
                    </td>
                    <td>${statusMap[r.status] || r.status}</td>
                    <td>${r.is_featured == 1 ? '<span class="badge badge-feat">&#9733; Nổi bật</span>' : '—'}</td>
                    <td>${formatDate(r.created_at)}</td>
                    <td class="actions">
                        <a href="them-thong-bao.php?id=${r.id}" class="btn btn-sm btn-outline">Sửa</a>
            ...
            // Pagination
            let pagHtml = `<span class="page-info">${total} thông báo</span>`;
            pagHtml += `<button ${page <= 1 ? 'disabled' : ''} onclick="loadNotifs(${page - 1})">&laquo;</button>`;
            for (let p = 1; p <= totalPages; p++) {
                pagHtml += `<button class="${p === page ? 'active' : ''}" onclick="loadNotifs(${p})">${p}</button>`;
            }
            pagHtml += `<button ${page >= totalPages ? 'disabled' : ''} onclick="loadNotifs(${page + 1})">&raquo;</button>`;
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            ...
            fetch('api/notifications.php?_method=DELETE', {
                    method: 'POST',
                    body: fd
                })
                .then(r => r.json())
        ...
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            searchTimer = setTimeout(() => loadNotifs(1), 350);
        });
        document.getElementById('statusFilter').addEventListener('change', () => loadNotifs(1));
