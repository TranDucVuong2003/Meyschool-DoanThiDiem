...
<body>
    ...
    <main class="body-content achievements-page">
        <div class="ac-container">
            ...
            <div class="ac-pagination" id="ac-pagination" style="display:flex;justify-content:center;gap:6px;margin-top:32px;flex-wrap:wrap;"></div>
        </div>
    </main>
                ...
                fetch('api/achievements-public.php?' + params)
                    ...
                    .then(function(res) {
                        ...
                        if (!res.success || !res.data || res.data.length === 0) {
                            grid.innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#999;padding:40px 0;">Chưa có thành tích nào.</p>';
                        ...
                        res.data.forEach(function(item) {
                            ...
                            html += '<a href="doc-thanh-tich.php?slug=' + encodeURIComponent(item.slug) + '" class="ac-card-link">';
                            html += '<div class="ac-card-img"><img src="' + escHtml(thumb) + '" alt="' + escHtml(item.title) + '"></div>';
                            if (item.year) {
                            ...
                            if (item.student_name) {
                                html += '<p class="ac-card-desc" style="font-size:12px;color:#888;">👤 ' + escHtml(item.student_name) + '</p>';
                            }
                            html += '</a>';
                            html += '</div>';
                        });
                        ...
                        if (res.totalPages <= 1) {
                            ...
                            var pagHtml = '';
                            if (currentPage > 1) {
                                pagHtml += '<button onclick="window._acLoad(' + (currentPage - 1) + ')" style="padding:6px 14px;border:1px solid #ddd;border-radius:6px;background:#fff;cursor:pointer;">‹</button>';
                            }
                            for (var p = 1; p <= res.totalPages; p++) {
                                var active = p === currentPage ? 'background:#2980b9;color:#fff;border-color:#2980b9;' : '';
                                pagHtml += '<button onclick="window._acLoad(' + p + ')" style="padding:6px 12px;border:1px solid #ddd;border-radius:6px;background:#fff;cursor:pointer;' + active + '">' + p + '</button>';
                            }
                    ...
                    .catch(function() {
                        document.getElementById('ac-grid').innerHTML = '<p style="grid-column:1/-1;text-align:center;color:#999;padding:40px 0;">Có lỗi khi tải dữ liệu.</p>';
                    });
            }
