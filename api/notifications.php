...
if (!dashboard_can_access('notifications')) {
...
if ($method === 'GET') {
    ...
    $dataStmt = $pdo->prepare("SELECT id, title, slug, thumbnail, status, is_featured, created_at, updated_at FROM notifications $whereSQL ORDER BY created_at DESC LIMIT $limit OFFSET $offset");
...
if ($method === 'POST' && empty($_GET['_method'])) {
    ...
    $check = $pdo->prepare("SELECT id FROM notifications WHERE slug = ?");
    $check->execute([$slug]);
    if ($check->fetch()) {
        $slug .= '-' . time();
    }
    ...
    $stmt = $pdo->prepare("INSERT INTO notifications (title, slug, excerpt, content, thumbnail, status, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$title, $slug, $excerpt, $content, $thumbnailPath, $status, $is_featured]);

    echo json_encode(['success' => true, 'message' => 'Tạo thông báo thành công', 'id' => $pdo->lastInsertId()]);
    exit;
}
...
if ($method === 'POST' && ($_GET['_method'] ?? '') === 'PUT') {
    ...
    $existing = $pdo->prepare("SELECT * FROM notifications WHERE id = ?");
    $existing->execute([$id]);
    $old = $existing->fetch();
    if (!$old) {
    ...
    $check = $pdo->prepare("SELECT id FROM notifications WHERE slug = ? AND id != ?");
    ...
    $stmt = $pdo->prepare("UPDATE notifications SET title=?, slug=?, excerpt=?, content=?, thumbnail=?, status=?, is_featured=? WHERE id=?");
    $stmt->execute([$title, $slug, $excerpt, $content, $thumbnailPath, $status, $is_featured, $id]);
...
if ($method === 'DELETE' || ($method === 'POST' && ($_GET['_method'] ?? '') === 'DELETE')) {
    ...
    $old = $pdo->prepare("SELECT thumbnail FROM notifications WHERE id = ?");
    $old->execute([$id]);
    $row = $old->fetch();
...
function uploadThumbnail($file)
{
    ...
    $uploadDir = __DIR__ . '/../storage/notifications/';
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0755, true);
    }

    $filename = 'notif-' . time() . '-' . bin2hex(random_bytes(4)) . '.' . $ext;
    $dest = $uploadDir . $filename;

    if (!move_uploaded_file($file['tmp_name'], $dest)) return false;

    return 'storage/notifications/' . $filename;
}
