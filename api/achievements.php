...
if (!dashboard_can_access('achievements')) {
...
if ($method === 'GET') {
    ...
    $dataStmt = $pdo->prepare("SELECT id, title, slug, thumbnail, year, award_level, student_name, status, is_featured, created_at FROM achievements $whereSQL ORDER BY created_at DESC LIMIT $limit OFFSET $offset");
...
if ($method === 'POST' && empty($_GET['_method'])) {
    ...
    $check = $pdo->prepare("SELECT id FROM achievements WHERE slug = ?");
    $check->execute([$slug]);
    if ($check->fetch()) {
        $slug .= '-' . time();
    }
    ...
    $stmt = $pdo->prepare("INSERT INTO achievements (title, slug, excerpt, content, thumbnail, year, award_level, student_name, status, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([$title, $slug, $excerpt, $content, $thumbnailPath, $year, $award_level, $student_name, $status, $is_featured]);

    echo json_encode(['success' => true, 'message' => 'Tạo thành tích thành công', 'id' => $pdo->lastInsertId()]);
    exit;
}
...
if ($method === 'POST' && ($_GET['_method'] ?? '') === 'PUT') {
    ...
    $existing = $pdo->prepare("SELECT * FROM achievements WHERE id = ?");
    $existing->execute([$id]);
    $old = $existing->fetch();
    if (!$old) {
        echo json_encode(['success' => false, 'message' => 'Thành tích không tồn tại']);
        exit;
    }
    ...
    $check = $pdo->prepare("SELECT id FROM achievements WHERE slug = ? AND id != ?");
    ...
    $stmt = $pdo->prepare("UPDATE achievements SET title=?, slug=?, excerpt=?, content=?, thumbnail=?, year=?, award_level=?, student_name=?, status=?, is_featured=? WHERE id=?");
    $stmt->execute([$title, $slug, $excerpt, $content, $thumbnailPath, $year, $award_level, $student_name, $status, $is_featured, $id]);
...
if ($method === 'DELETE' || ($method === 'POST' && ($_GET['_method'] ?? '') === 'DELETE')) {
    ...
    $old = $pdo->prepare("SELECT thumbnail FROM achievements WHERE id = ?");
    $old->execute([$id]);
    $row = $old->fetch();
    ...
    echo json_encode(['success' => true, 'message' => 'Đã xóa thành tích']);
    exit;
}
...
function uploadThumbnail($file)
{
    ...
    $uploadDir = __DIR__ . '/../storage/achievements/';
    if (!is_dir($uploadDir)) {
    ...
    return 'storage/achievements/' . $filename;
}
