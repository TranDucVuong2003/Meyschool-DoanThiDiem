...
$isEdit = $editId > 0;
$pageTitle = $isEdit ? 'Sửa thành tích' : 'Thêm thành tích mới';
$_sidebar_page = 'thanh-tich-admin.php';
?>
<!DOCTYPE html>
<html lang="vi">
...
<body>
    ...
    <div class="main">
        ...
        <div class="content">
            <div class="breadcrumb">
                <a href="thanh-tich-admin.php">Thành tích</a> &rsaquo; <?= $pageTitle ?>
            </div>
            ...
            <form id="achForm" enctype="multipart/form-data">
                <input type="hidden" name="id" id="achId" value="<?= $editId ?>">
                <div class="form-grid">
                    <!-- Cột chính -->
                    <div>
                        <div class="card">
                            <div class="card-title">Nội dung thành tích</div>
                            <div class="form-group">
                                <label>Tiêu đề <span class="req">*</span></label>
                                <input type="text" name="title" id="titleInput" placeholder="Nhập tiêu đề thành tích..." required>
                            </div>
                            <div class="form-group">
                        ...
                        <div class="card" style="margin-bottom:18px;">
                            ...
                            <div class="form-group">
                                <div class="check-group">
                                    ...
                                    <label for="featuredCheck">Thành tích nổi bật</label>
                                </div>
                            </div>
                            <div class="form-actions">
                                <a href="thanh-tich-admin.php" class="btn btn-outline">Hủy</a>
                                <button type="submit" class="btn btn-primary" id="saveBtn">
                                    <?= $isEdit ? 'Cập nhật' : 'Tạo thành tích' ?>
                                </button>
                            </div>
                        </div>

                        <div class="card" style="margin-bottom:18px;">
                            <div class="card-title">Thông tin giải thưởng</div>
                            <div class="form-group">
                                <label>Năm học</label>
                                <input type="number" name="year" id="yearInput" placeholder="VD: 2024" min="2000" max="2099">
                            </div>
                            <div class="form-group">
                                <label>Tên giải thưởng / Hạng mục</label>
                                <input type="text" name="award_level" id="awardInput" placeholder="VD: Giải Nhất học sinh giỏi cấp Thành phố">
                            </div>
                            <div class="form-group">
                                <label>Tên học sinh (nếu có)</label>
                                <input type="text" name="student_name" id="studentInput" placeholder="VD: Nguyễn Văn A">
                            </div>
                        </div>
    ...
    <script>
        ...
        if (isEdit) {
            fetch('api/achievements.php?id=' + editId)
                .then(r => r.json())
                .then(res => {
                    if (!res.success) {
                        showToast('Không tìm thấy thành tích', 'error');
                        return;
                    }
                    const d = res.data;
                    ...
                    document.getElementById('yearInput').value = d.year || '';
                    document.getElementById('awardInput').value = d.award_level || '';
                    document.getElementById('studentInput').value = d.student_name || '';
                    if (d.thumbnail) {
            ...
            fetch(url, {
                ...
                .then(res => {
                    ...
                    if (res.success) {
                        ...
                        if (!isEdit) setTimeout(() => window.location.href = 'thanh-tich-admin.php', 1000);
                    } else {
