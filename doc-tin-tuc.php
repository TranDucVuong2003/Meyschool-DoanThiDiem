...
<body>
            ...
            <div class="detail-container detail-grid">
                ...
                <aside class="detail-sidebar">
                    <div class="sidebar-box">
                        <h3 class="sidebar-box-title">Tin tức mới nhất</h3>
                        <div class="latest-news-list" id="latestNewsList">
                            <p style="font-size:13px;color:#999;">Đang tải...</p>
                        </div>
                        <a href="tin-tuc.php" class="sidebar-see-all">TẤT CẢ TIN TỨC ►</a>
                    </div>

                    <div class="sidebar-box">
                        <h3 class="sidebar-box-title">Danh mục</h3>
                        <ul class="category-list" id="categoryList">
                            <li><a href="tin-tuc.php">Tất cả</a></li>
                        </ul>
                    </div>
                </aside>
            </div>
    ...
    <script>
        (function() {
            ...
            if (!slug) {
                document.getElementById('pageTitleMain').textContent = 'Không tìm thấy bài viết';
                document.getElementById('articleContent').innerHTML =
                    '<p style="text-align:center;color:#999;">Bài viết không tồn tại hoặc đã bị xóa.</p>';
                return;
            }
            ...
            // Load article
            $.getJSON('api/news-public.php?slug=' + encodeURIComponent(slug), function(res) {
                ...
                document.getElementById('breadcrumbTitle').textContent = a.category_name || 'Tin tức';
                ...
                if (a.category_name) {
                    document.getElementById('articleTags').innerHTML =
                        '<a class="detail-tag" href="tin-tuc.php">' + escHtml(a.category_name) + '</a>';
                }

                // Load related articles (same category, exclude current)
                if (a.category_id) {
                    $.getJSON('api/news-public.php?category_id=' + a.category_id + '&page=1', function(rel) {
                        if (rel.success && rel.data && rel.data.length > 0) {
                            ...
                            if (links.length) {
                                ...
                                links.forEach(function(r) {
                                    listHtml += '<li><a href="doc-tin-tuc.php?slug=' +
            ...
            $.getJSON('api/news-public.php?page=1', function(res) {
                if (res.success && res.data && res.data.length) {
                    ...
                    res.data.slice(0, 6).forEach(function(r) {
                        html += '<a class="latest-news-item" href="doc-tin-tuc.php?slug=' +
                    ...
                    document.getElementById('latestNewsList').innerHTML = html;
                }
            });
