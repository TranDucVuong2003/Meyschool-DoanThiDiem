...
<head>
    ...
    <title>Quản lý Tin tức | Meyschool Đoàn Thị Điểm</title>
...
<body>
    ...
    <div class="main">
        ...
        <div class="content">
            <div class="page-header">
                <h2>Tin tức</h2>
                <a href="them-tin-tuc.php" class="btn btn-primary">+ Thêm bài viết</a>
            </div>
    ...
    <div class="modal-overlay" id="deleteModal">
        <div class="modal-box">
            <h3>Xác nhận xóa</h3>
            <p>Bạn có chắc muốn xóa bài viết này? Hành động không thể hoàn tác.</p>
    ...
    <script>
        ...
        function renderTable(rows, total, page, totalPages) {
            ...
            if (rows.length === 0) {
                tbody.innerHTML = '<tr><td colspan="7" class="empty-state">Chưa có bài viết nào.</td></tr>';
            ...
            tbody.innerHTML = rows.map((r, i) => `
                <tr>
                    ...
                    <td class="title-cell"><a href="them-tin-tuc.php?id=${r.id}">${escHtml(r.title)}</a></td>
                    <td>${statusMap[r.status] || r.status}</td>
                    <td>${r.is_featured == 1 ? '<span class="badge badge-feat">&#9733; Nổi bật</span>' : '—'}</td>
                    <td>${formatDate(r.created_at)}</td>
                    <td class="actions">
                        <a href="them-tin-tuc.php?id=${r.id}" class="btn btn-sm btn-outline">Sửa</a>
            ...
            // Pagination
            let pagHtml = '';
            pagHtml += `<span class="page-info">${total} bài viết</span>`;
            pagHtml += `<button ${page <= 1 ? 'disabled' : ''} onclick="loadNews(${page - 1})">&laquo;</button>`;
            for (let p = 1; p <= totalPages; p++) {
                pagHtml += `<button class="${p === page ? 'active' : ''}" onclick="loadNews(${p})">${p}</button>`;
            }
            pagHtml += `<button ${page >= totalPages ? 'disabled' : ''} onclick="loadNews(${page + 1})">&raquo;</button>`;
        ...
        document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
            ...
            fetch('api/news.php?_method=DELETE', {
                    method: 'POST',
                    body: fd
                })
                .then(r => r.json())
        ...
        document.getElementById('searchInput').addEventListener('input', function() {
            ...
            searchTimer = setTimeout(() => loadNews(1), 350);
        });
        document.getElementById('statusFilter').addEventListener('change', () => loadNews(1));
