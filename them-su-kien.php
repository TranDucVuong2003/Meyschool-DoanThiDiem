...
$isEdit    = $editId > 0;
$pageTitle = $isEdit ? 'Sửa sự kiện' : 'Thêm sự kiện mới';
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card-title { font-size: 15px; font-weight: 700; color: var(--primary-d); margin-bottom: 18px; padding-bottom: 12px; border-bottom: 1px solid var(--border); }
        ...
        .form-group textarea,
        .form-group select { width: 100%; padding: 10px 14px; border: 1px solid var(--border); border-radius: 8px; font-size: 14px; font-family: inherit; color: var(--text); background: #fff; outline: none; transition: border-color .2s; }
        ...
        .slug-row button { padding: 10px 14px; border: 1px solid var(--border); border-radius: 8px; background: var(--bg); cursor: pointer; font-size: 12px; color: var(--muted); white-space: nowrap; }
        .slug-row button:hover { background: var(--border); }

        .char-count { font-size: 11px; color: var(--muted); text-align: right; margin-top: 4px; }

        .thumb-upload { border: 2px dashed var(--border); border-radius: 10px; padding: 24px; text-align: center; cursor: pointer; transition: border-color .2s, background .2s; position: relative; }
        .thumb-upload:hover { border-color: var(--accent); background: #f8fbfe; }
        .thumb-upload input[type="file"] { position: absolute; inset: 0; opacity: 0; cursor: pointer; }
        .thumb-upload .upload-icon { font-size: 36px; color: var(--muted); margin-bottom: 8px; }
        .thumb-upload .upload-text { font-size: 13px; color: var(--muted); }
        .thumb-preview { margin-top: 12px; position: relative; display: none; }
        .thumb-preview img { width: 100%; border-radius: 8px; max-height: 200px; object-fit: cover; }
        .thumb-remove { position: absolute; top: 6px; right: 6px; width: 28px; height: 28px; border-radius: 50%; background: rgba(231,76,60,.9); color: #fff; border: none; cursor: pointer; font-size: 14px; display: flex; align-items: center; justify-content: center; }
        ...
        .check-group label { font-size: 13px; margin: 0; }
        ...
        .saving-overlay { display: none; position: fixed; inset: 0; background: rgba(255,255,255,.6); z-index: 400; align-items: center; justify-content: center; font-size: 16px; color: var(--primary-d); font-weight: 600; }
        .saving-overlay.show { display: flex; }

        @media (max-width: 900px) { .form-grid { grid-template-columns: 1fr; } }
        @media (max-width: 700px) { .main { margin-left: 0; } }
    </style>
...
<body>
    ...
    <div class="main">
        ...
        <div class="content">
            <div class="breadcrumb">
                <a href="su-kien-admin.php">Sự kiện trường</a> &rsaquo; <?= $pageTitle ?>
            </div>
            ...
            <form id="eventForm" enctype="multipart/form-data">
                <input type="hidden" name="id" id="eventId" value="<?= $editId ?>">
                <div class="form-grid">
                    <!-- Cột chính -->
                    <div>
                        <div class="card">
                            <div class="card-title">Nội dung sự kiện</div>
                            <div class="form-group">
                                <label>Tên sự kiện <span class="req">*</span></label>
                                <input type="text" name="title" id="titleInput" placeholder="Nhập tên sự kiện..." required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="event_location" id="locationInput" placeholder="VD: Sân trường, Hội trường A...">
                            </div>
                            <div class="form-group">
                                <label>Tóm tắt</label>
                    ...
                    <div>
                        ...
                        <div class="card" style="margin-bottom:18px;">
                            <div class="card-title">Thời gian sự kiện</div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                <label>Ngày kết thúc <small style="color:var(--muted);font-weight:400;">(nếu nhiều ngày)</small></label>
                                <input type="date" name="event_end_date" id="eventEndDate">
                            </div>
                        </div>
                        ...
                        <div class="card" style="margin-bottom:18px;">
                            ...
                            <div class="form-group">
                                <div class="check-group">
                                    ...
                                    <label for="featuredCheck">Sự kiện nổi bật</label>
                                </div>
                            </div>
                            <div class="form-actions">
                                <a href="su-kien-admin.php" class="btn btn-outline">Hủy</a>
    ...
    <script>
        ...
        var editorInstance = CKEDITOR.replace('contentInput', {
            language: 'vi', height: 400, removePlugins: 'elementspath',
            toolbar: [
        ...
        // Load data for edit
        if (isEdit) {
            fetch('api/events.php?id=' + editId)
                .then(r => r.json())
                .then(res => {
                    if (!res.success) { showToast('Không tìm thấy sự kiện', 'error'); return; }
                    const d = res.data;
                    document.getElementById('titleInput').value   = d.title          || '';
                    document.getElementById('slugInput').value    = d.slug           || '';
                    document.getElementById('locationInput').value= d.event_location || '';
                    ...
                    document.getElementById('eventEndDate').value = d.event_end_date || '';
        ...
        document.getElementById('eventForm').addEventListener('submit', function(e) {
            ...
            if (!document.getElementById('featuredCheck').checked) fd.set('is_featured','0');

            let url = 'api/events.php';
            if (isEdit) { url += '?_method=PUT'; fd.set('id', editId); }
            ...
            fetch(url, { method: 'POST', body: fd })
                ...
                .then(res => {
                    ...
                    if (res.success) {
                        ...
                        if (!isEdit) setTimeout(() => window.location.href = 'su-kien-admin.php', 1000);
                    } else {
