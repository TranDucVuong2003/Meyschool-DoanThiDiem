    ...
    <script>
        ...
        function statusOptions(currentStatus) {
            return '' +
                ...
                '<option value="da_lien_he"' + (currentStatus === 'da_lien_he' ? ' selected' : '') + '>Đã liên hệ</option>' +
                '<option value="da_xep_lich"' + (currentStatus === 'da_xep_lich' ? ' selected' : '') + '>Đã xếp lịch</option>' +
                '<option value="da_tham_quan"' + (currentStatus === 'da_tham_quan' ? ' selected' : '') + '>Đã tham quan</option>' +
        ...
        function renderTable(rows, total, page, totalPages) {
            ...
            tbody.innerHTML = rows.map((r, i) => {
                const shortMsg = (r.message || '').length > 45 ? (r.message || '').slice(0, 45) + '...' : (r.message || '—');
                ...
                return '<tr>' +
                    ...
                    '<td>' +
                    '<select class="status-select status-' + (r.status || 'moi') + '" onchange="updateStatus(' + r.id + ', this.value, this)">' +
                    statusOptions(r.status || 'moi') +
                    '</select>' +
                    '</td>' +
                    '<td style="text-align:center;padding:8px 16px;" class="' + unreadClass + '">' +
                    '<input type="checkbox" class="read-checkbox" ' + (Number(r.is_read) ? 'checked' : '') + ' onchange="markRead(' + r.id + ', this.checked)">' +
                    '</td>' +
            ...
            pagHtml += '<button ' + (page <= 1 ? 'disabled' : '') + ' onclick="loadTours(' + (page - 1) + ')">&laquo;</button>';
            for (let p = 1; p <= totalPages; p++) {
                pagHtml += '<button class="' + (p === page ? 'active' : '') + '" onclick="loadTours(' + p + ')">' + p + '</button>';
            }
            pagHtml += '<button ' + (page >= totalPages ? 'disabled' : '') + ' onclick="loadTours(' + (page + 1) + ')">&raquo;</button>';
